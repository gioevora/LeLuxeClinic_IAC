...
const AppointmentForm = () => {
  ...
  const validationSchema = Yup.object({
    ...
    serviceId: Yup.number()
      ...
      .oneOf(filteredServices.map(service => service.id), 'Service is required'),
    message: Yup.string(),
  });
  ...
  useEffect(() => {
    if (selectedCategory) {
      setFilteredServices(
        services.filter((service) => service.categoryId === Number(selectedCategory))
      );
    } else {
      setFilteredServices([]);
    }
      ...
      >
        {({ isSubmitting, setFieldValue, values }) => (
          <Form className="py-4 px-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="mb-4 col-span-2 md:col-span-1">
              <label className="block text-gray-700 font-bold mb-2" htmlFor="serviceCategory">
                Service Category
              </label>
              <Field
                as="select"
              ...
              >
                ...
                {services
                  ...
                  .map((service) => (
                    <option key={service.category?.id} value={service.category?.id}>
            ...
            {selectedCategory && filteredServices.length > 0 && (
              <div className="mb-4 col-span-2">
                ...
                <ErrorMessage name="serviceId" component="div" className="text-red-500 text-sm" />
              </div>
            )}
